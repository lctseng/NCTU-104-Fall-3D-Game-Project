...
void GameConsoleWindow::CreateCEGUIWindow()
{
	...
	m_ConsoleWindow = pWindowManager->loadLayoutFromFile("MyConsole.layout");
...
void GameConsoleWindow::setVisible(bool visible)
{
    ...
    CEGUI::Editbox* editBox = static_cast<CEGUI::Editbox*>(m_ConsoleWindow->getChild(sNamePrefix + "EditBox"));
    if(visible)
       editBox->activate();
    else
       editBox->deactivate();
